...
```bash
pnpm test                 # full Vitest suite (file-scoped during dev: pnpm test <file>)
pnpm lint                 # ESLint — zero warnings
pnpm typecheck            # tsc --noEmit (strict)
pnpm build                # electron-builder (.dmg / .exe)
pnpm install | dev | build | test | lint | typecheck | format   # full reference
```
...
**Invocation method:** **B (Sentinel-in-CI)** — enforced by branch protection on `main` (required `sentinel` status check) for production merges — **plus A (full-capability sub-agent, §How to Invoke)** during development. Method C (manual) is not used; CI and sub-agent runs apply this same ruleset.
...
- **Formatter**: Prettier — run before commit. **Linter**: ESLint with typescript-eslint strict (+ eslint-plugin-react, jsx-a11y) — fix all warnings.
- Conventions: named exports; functional React components; isolated connector modules behind a common importer interface; all archive extraction zip-slip / path-traversal guarded; heavy ingestion off the UI thread (worker threads / ffmpeg subprocess); streaming parses for large exports; strict local-only — zero network egress
...
- **Project (local-only)**: transmit/upload/sync any user memory data off the device · add telemetry or analytics on user content · weaken or remove the local-only guarantee, Sentinel, tests, branch protection, or the security scanners · add any network egress, backend, account, or cloud sync without a human-required gate
