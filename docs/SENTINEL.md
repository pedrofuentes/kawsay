...
**Invocation method (this project):** **B — Sentinel-in-CI**, enforced by branch protection on `main` (required `sentinel` status check), gates production merges; **plus A — full-capability sub-agent** (AGENTS.md §How to Invoke) during development. Method C (manual) is not used. CI and sub-agent invocations apply this identical ruleset.
