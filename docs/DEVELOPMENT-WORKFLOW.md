...
**Prerequisites:** Node.js 20+ and pnpm 9 (`corepack enable` pins pnpm).

```bash
pnpm install        # install dependencies
pnpm dev            # run the app (Electron + Vite dev server)
pnpm test           # run Vitest once
pnpm test:watch     # Vitest in watch mode
pnpm lint           # ESLint
pnpm typecheck      # tsc --noEmit (strict)
pnpm build          # package installers via electron-builder (.dmg / .exe)
```

No environment variables or secrets are required for local development — Kawsay is fully offline and stores everything on-device.
