...
ADRs are listed in reverse chronological order (newest first). For context on the ADR format and authorization tiers, see [`../DECISIONS.md`](../DECISIONS.md).
...
| **ADR-0027** | [M2 on-device transcription — whisper.cpp via bundled binary + opt-in model download](./0027-m2-on-device-transcription-whisper-cpp-bundled-binary-opt-in-model-download.md) | Proposed — HUMAN-REQUIRED |
| **ADR-0026** | [Release pipeline — GitHub Actions workflow publishing unsigned v1 installers](./0026-release-pipeline-github-actions-workflow-publishing-unsigned-v1-installers-to-github-releases.md) | Accepted (implements ADR-0007; complements ADR-0023/0024/0025) |
| **ADR-0025** | [Defer enableEmbeddedAsarIntegrityValidation to the signed build](./0025-defer-enableembeddedasarintegrityvalidation-to-the-signed-build.md) | Accepted (refines ADR-0007; v1 exception to ARCHITECTURE §2.5) |
| **ADR-0024** | [Bump better-sqlite3 12.9.0 → 12.11.1 for Electron 42 native compatibility](./0024-bump-better-sqlite3-1290-12111-for-electron-42-native-compatibility.md) | Accepted (enables ADR-0007 / card P1, AC-5) |
| **ADR-0023** | [Packaging finalization — fuse-flip mechanism, non-publishing auto-builds, ABI ordering](./0023-packaging-finalization-card-p1-fuse-flip-mechanism-non-publishing-auto-builds-abi-ordering.md) | Accepted (refines ADR-0007) |
...
| **ADR-0013** | [Revert Takeout email tooling to `mailparser` + in-module streaming splitter](./0013-revert-takeout-email-tooling-to-mailparser-streaming-splitter.md) | Accepted |
| **ADR-0012** | [Media-ingestion dependencies + off-thread ingestion engine](./0012-media-ingestion-dependencies-off-thread-ingestion-engine.md) | Accepted |
...
| **ADR-0009** | [Takeout `.mbox` streaming split + email-parser substitution](./0009-takeout-mbox-streaming-split-email-parser-substitution-mailparser-mbox-parser-postal-mime.md) | Superseded by ADR-0013 |
| **ADR-0008** | [Privacy, data location & the local-only / zero-egress invariant](./0008-privacy-data-location-the-local-only-zero-egress-invariant.md) | Proposed — HUMAN-REQUIRED |
...
| **ADR-0003** | [Local catalog — better-sqlite3 schema + migration runner + dedup-with-provenance](./0003-local-catalog-better-sqlite3-schema-migration-runner-originals-on-disk-dedup-with-provenance.md) | Accepted — HUMAN-REQUIRED gate |
...
> **Note**: ADR-0018 and ADR-0019 are reserved but not yet documented. The **Status** column mirrors each ADR file's own `**Status**:` line — some decisions are **Proposed** (and may carry a HUMAN-REQUIRED sign-off gate) or **Superseded**, not yet **Accepted**.
...
3. Add an entry to this index in reverse chronological (newest-first) order, matching the file's own `**Status**:` line.
