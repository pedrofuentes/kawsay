...
### Mocking
Prefer **dependency injection**: pass collaborators (filesystem, the SQLite catalog, ffmpeg/ffprobe runners, parsers) into modules so tests supply lightweight fakes. For module-level boundaries use Vitest `vi.mock` / `vi.fn`. Unit tests never touch the real network, real user data, or real external binaries — the local-only / zero-egress invariant is asserted by a dedicated test, not mocked away.

```typescript
// Example: How to mock in this project
import { describe, it, expect, vi } from 'vitest';
import { importWhatsAppExport } from '../src/main/import/whatsapp';

// Mock the archive extractor so the importer is exercised in isolation.
vi.mock('../src/main/archive/extractZip', () => ({
  extractZip: vi.fn().mockResolvedValue(['chat.txt', 'IMG-0001.jpg', 'AUD-0002.opus']),
}));

const items = await importWhatsAppExport('/exports/whatsapp.zip');
...
describe('WhatsAppImporter', () => {
  it('should catalog voice notes when the export contains audio files', () => {
    // Arrange → Act → Assert
  });
});
```
