...
| **AC-28** Instagram direct messages JSON | `importers/instagram-importer.ts` + shared Meta DYI parser | unit + integration |
...
| `instagram` | Meta JSON folder / `.zip` (`your_instagram_activity/messages/inbox/.../message_*.json`) | shared Meta DYI parser, cheap Instagram-root detection, bounded streaming parse, mojibake fix, Unix ms → UTC | messages (`none`) plus linked photo/video/audio originals copied once, content-addressed | AC-28, AC-4, AC-15 |
