...
const ROOT_DIR = 'your_activity_across_facebook';
const MESSAGES_DIR = 'messages';
const BUCKETS = ['inbox', 'archived_threads', 'filtered_threads'] as const;
...
function stripRoot(path: string): string {
  ...
  const rootPrefix = `${ROOT_DIR}/`;
  return posix.startsWith(rootPrefix) ? posix.slice(rootPrefix.length) : posix;
}

function messengerRel(path: string): string | null {
  const stripped = stripRoot(path);
  const prefix = `${MESSAGES_DIR}/`;
  return stripped.startsWith(prefix) ? stripped.slice(prefix.length) : null;
}

function isMessageFile(path: string): boolean {
  const rel = messengerRel(path);
  if (rel === null) return false;
  const parts = rel.split('/');
  return (
    parts.length === 3 &&
    BUCKETS.includes(parts[0] as (typeof BUCKETS)[number]) &&
...
function looksLikeMessengerThread(prefix: string): boolean {
...
async function findMessengerMarker(inputPath: string, deps: ImporterDeps): Promise<string | null> {
  for (const root of [join(inputPath, ROOT_DIR, MESSAGES_DIR), join(inputPath, MESSAGES_DIR)]) {
  ...
  if (isZip(inputPath)) {
    try {
      ...
      recordSkip(
        ...
        skipped,
        inputPath,
        `could not extract Messenger archive: ${errorMessage(error)}`,
        'E_EXTRACT',
      );
...
function parseThreadMeta(prefix: string, rel: string): MessengerThreadMeta {
  const parts = rel.split('/');
  const fallback: MessengerThreadMeta = {
...
async function* streamMessages(
  ...
  const rel = messengerRel(entry.entryPath) ?? entry.entryPath;
  ...
  let token = '';
  let prefix = '';
  let meta: MessengerThreadMeta | null = null;
  let objectDepth = 0;
  let objectText = '';
          ...
          if (!objectTooLarge) {
            ...
            if (objectText.length > MAX_MESSAGE_JSON_CHARS) {
              ...
              recordSkip(
                ...
                skipped,
                rel,
                'message JSON object exceeded the Messenger size cap',
                'E_MESSAGE_TOO_LARGE',
              );
            }
          }
...
function buildMediaResolver(
  ...
  for (const entry of entries) {
    const stripped = stripRoot(entry.entryPath);
...
function messageRecord(rel: string, streamed: StreamedMessage): CatalogRecord {
...
export const messengerImporter: Importer = {
    ...
    try {
      if (isZip(inputPath)) {
        return await zipHasEntryName(inputPath, [
          `${ROOT_DIR}/${MESSAGES_DIR}/inbox/`,
          `${MESSAGES_DIR}/inbox/`,
        ]);
      }
      const root = await deps.fs.stat(inputPath);
      if (!root.isDirectory()) return false;
      const marker = await findMessengerMarker(inputPath, deps);
      if (marker === null) return false;
      return looksLikeMessengerThread(await readMarkerPrefix(marker, deps));
    } catch {
      return false;
    }
  },
  ...
  async *import(
    ...
    const { entries, failed } = await gatherEntries(inputPath, ctx, skipped);
    if (failed) return { recordCount, skipped };
    const messageFiles = entries
      .filter((entry) => isMessageFile(entry.entryPath))
      .sort((a, b) => {
        const aRel = messengerRel(a.entryPath) ?? a.entryPath;
        const bRel = messengerRel(b.entryPath) ?? b.entryPath;
        const aBucket = aRel.split('/')[0] ?? '';
        const bBucket = bRel.split('/')[0] ?? '';
        const bucketDiff =
          BUCKETS.indexOf(aBucket as (typeof BUCKETS)[number]) -
          BUCKETS.indexOf(bBucket as (typeof BUCKETS)[number]);
        return bucketDiff === 0 ? aRel.localeCompare(bRel) : bucketDiff;
      });
    const resolveMedia = buildMediaResolver(entries);
    ...
    for (const entry of messageFiles) {
      ...
      const rel = messengerRel(entry.entryPath) ?? entry.entryPath;
      try {
        for await (const streamed of streamMessages(entry, ctx, skipped)) {
          if (ctx.signal.aborted) return { recordCount, skipped };
          ...
          const text = messageRecord(rel, streamed);
          recordCount += 1;
          ctx.onProgress({ phase: 'emit', processed: recordCount, total: null });
          yield text;
        }
        ...
        recordSkip(
          ...
          skipped,
          rel,
          `could not read Messenger thread: ${errorMessage(error)}`,
          'E_READ_JSON',
        );
      }
    }
