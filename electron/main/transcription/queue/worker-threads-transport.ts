...
// agnostic coordinator/job onto a real OS worker thread (AC-18). worker_threads
// (rather than utilityProcess) keeps the heavy whisper-cli orchestration off the UI
// thread while remaining fully unit-testable: every adapter is structural, so a fake
// Worker/MessagePort drives them in-process, and a real thread exercises the same
// code end-to-end. Mirrors the F3c ingestion transport seam. Both directions are
// raw structured-clone messages.
...
export function createWorkerThreadsHostHandle(worker: WorkerLike): TranscriptionWorkerHandle {
  return {
    post: (message) => worker.postMessage(message),
    onMessage: (handler) => worker.on('message', (value) => handler(value as WorkerToHostMessage)),
    // Subscribing to 'error' is what stops a worker fault (a throw outside the job
    // try/catch — module-load failure, native crash, OOM) from propagating to the
    // host as an uncaughtException and crashing the main process. 'exit' lets the
    // coordinator settle a batch whose worker died without ever sending a terminal
    // message, so the handle is never orphaned.
...
export interface WorkerThreadsSpawnerOptions {
  /** Absolute path to the built worker entry (out/main/transcription-worker.js). */
  scriptPath: string;
  /** Injectable worker factory (defaults to a real worker_threads Worker). */
  createWorker?: (scriptPath: string) => WorkerLike;
}

/** A {@link SpawnTranscriptionWorker} that forks a fresh worker thread per batch. */
